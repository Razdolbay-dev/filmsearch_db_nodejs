<template>
  <div>
    <!-- Кнопка поиска для сериала -->
    <button
        @click="openSearchModal"
        class="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-medium py-2 px-4 rounded-lg transition-all duration-200 transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 mt-2"
        :disabled="loading"
    >
          ...
          <div class="relative w-full max-w-7xl bg-white rounded-xl shadow-2xl transform transition-all">
                ...
                <div class="flex items-center justify-between">
                  <div class="flex-1">
                    <h3 class="text-xl font-semibold text-gray-900 flex items-center gap-2">
                      ...
                      Результаты поиска торрентов для сериала
                    </h3>
                    <p class="mt-1 text-sm text-gray-500">
                      <span class="font-medium">{{ series.name }}</span>
                      <span v-if="series.original_name && series.original_name !== series.name" class="text-gray-400">
              ...
              <div class="px-6 py-3 bg-gray-50 border-t border-b border-gray-200">
                <div class="flex flex-wrap items-center gap-3">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-medium text-gray-700">Сезон:</span>
                  </div>

                  <select v-model="selectedSeason" class="px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
            ...
            <div class="max-h-[calc(100vh-200px)] overflow-y-auto">
              <div v-if="loading" class="flex flex-col items-center justify-center py-12">
                ...
                <p class="mt-3 text-gray-500">Поиск торрентов для сериала...</p>
              ...
              <div v-else-if="filteredResults.length === 0" class="flex flex-col items-center justify-center py-12">
                ...
                <p class="text-sm text-gray-400 mt-1">Попробуйте изменить параметры поиска или выбрать другой сезон</p>
              </div>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                      <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 flex-wrap mb-2">
                          ...
                          <span v-if="detectSeason(result.Title)" class="px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded-full">
                            {{ detectSeason(result.Title) }}
                          </span>
                        </div>
            ...
            <div class="sticky bottom-0 bg-gray-50 rounded-b-xl border-t border-gray-200 px-6 py-3">
              <div class="flex items-center justify-between text-sm text-gray-600">
                <div class="flex items-center gap-4">
                  ...
                  <span v-if="selectedSeason" class="text-gray-300">|</span>
                  <span v-if="selectedSeason">Сезон: <span class="font-semibold">{{ selectedSeason }}</span></span>
                </div>
                <button
                    @click="closeModal"
...
// Доступные сезоны (от 1 до количества сезонов)
const availableSeasons = computed(() => {
  const seasons = props.series.number_of_seasons || 1;
...
const detectSeason = (title) => {
  const seasonMatch = title.match(/[Ss](\d{1,2})/);
  if (seasonMatch) {
    return `${parseInt(seasonMatch[1])} сезон`;
  }
  return null;
};
...
const filteredResults = computed(() => {
  ...
  if (selectedSeason.value) {
    filtered = filtered.filter(result => {
      const season = detectSeason(result.Title);
      if (season) {
        return season.includes(selectedSeason.value);
      }
      // Если сезон не указан в названии, но выбран конкретный сезон - исключаем
      return false;
    });
  }
...
// Выполнить поиск для сериала
const performSearch = async () => {
  loading.value = true;

  try {
    const response = await apiClient.searchTorrentsForSeries(props.series);
...
// Поиск по конкретному сезону
const searchBySeason = async (season) => {
  selectedSeason.value = season;
  loading.value = true;

  try {
    const response = await apiClient.searchTorrentsForSeries(props.series, season);
...
const trackDownload = (result) => {
  emit('download-track', {
    series: props.series,
    torrent: result,
    season: detectSeason(result.Title),
